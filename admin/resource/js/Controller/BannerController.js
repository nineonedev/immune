...
export class BannerController {
  ...
  init() {
    ...
    this.toggleLinkInputOnLoad();
    this.watchLinkToggle();
  }
  ...
  toggleLinkInputOnLoad() {
    this.toggleLinkInput();
  }

  watchLinkToggle() {
    const radios = document.querySelectorAll('input[name="has_link"]');
    radios.forEach((radio) => {
      radio.addEventListener("change", () => this.toggleLinkInput());
    });
  }

  toggleLinkInput() {
    const hasLink = document.querySelector(
      'input[name="has_link"]:checked'
    )?.value;
    const linkBlock = document.getElementById("link_url_block");
    if (linkBlock) {
      linkBlock.style.display = hasLink === "1" ? "flex" : "none";
    }
  }
}
