...
<body>
	<div class="no-wrap">
        ...
        <main class="no-app no-container">
            ...
            ## 시작월 구함
            $result=mysql_query("SELECT MIN(Month) as CDMM FROM nb_counter_data WHERE Year='$Start_Year'");
            $data = mysql_fetch_array($result);
                                ...
                                <div class="no-admin-block no-w-80">
                                    ...
                                    <div class="no-admin-content no-admin-date">
                                        <div class="no-search-wrap">
                                            <input type="text" name="sdate" id="sdate" value="<?=$curYM?>" autocomplete="off">
                                ...
                                <div class="no-table-responsive">
                                    <table class="no-table">
                                        ...
                                        <?
                                            ...
                                            ## 최대방문자수 구함
                                            $result=mysql_query("SELECT ifnull(MAX(Visit_Num),0) as MV FROM nb_counter_data WHERE Year='$Select_Year' AND month='$Select_Month' ");
                                            $data = mysql_fetch_array($result);
                                            $max = $data[MV];
                                            if(!$max) $max = 0;

                                            for($i=1 ; $i<=31 ; $i++){
                                                $result=mysql_query("SELECT ifnull(SUM(Visit_Num),0) as SV FROM nb_counter_data WHERE Year='$Select_Year' AND month='$Select_Month' AND Day='$i'");
                                                $data = mysql_fetch_array($result);
                                                $Month_Num = $data[SV];
