...
$(function () {
  ...
  const $userPwd = $signupWrap.find("#password");

  const $userName = $signupWrap.find("#name");

  const $userPhone = $signupWrap.find("#phone");

  const $userEmail = $signupWrap.find("#email");
...
$(function () {
  ...
  const $inputs = $wrap.find(
    "#user_name, #user_phone, #user_email, #user_birth"
  );
...
$(function () {
  ...
  function togglePwdActive() {
    ...
    const isAllFilled =
      oldVal.length > 0 && newVal.length > 0 && confirmVal.length > 0;
